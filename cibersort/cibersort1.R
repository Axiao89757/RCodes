...
sig_matrix = 'LM22.txt'
mixture_file = 'ex2.csv'
# 主要�? read.table 的参数可以随意修改，根据你自己的表达矩阵txt文件适应性调整即�?
X <- read.table(sig_matrix,header=T,sep="\t",row.names=1,check.names=F)
Y <- read.table(mixture_file, header=T, sep="\t", check.names=F)

# 运行 CIBERSORT 会写�? CIBERSORT-Results.txt 文件覆盖掉之前的结果
# 务必看清�? test.txt 这个表达矩阵格式哦！！！
# (sig_matrix, mixture_file, perm=10, QN=TCIBERSORTRUE)
# 后面可以�? CIBERSORT-Results.txt 里面的结果进行各式各样的可视�?
